...
import team.abnormals.neutronia.client.gui.ProgressManager;
...
public class SplashProgress
{
    ...
    {
        File configFile = new File(MinecraftClient.getInstance().runDirectory, "config/splash.properties");
        ...
        final Identifier fontLoc = new Identifier(getString("fontTexture", "textures/font/ascii.png"));
        final Identifier logoLoc = new Identifier("textures/gui/title/mojang.png");
        final Identifier forgeLoc = new Identifier(getString("forgeTexture", "fml:textures/gui/forge.png"));
        final Identifier forgeFallbackLoc = new Identifier("fml:textures/gui/forge.png");

        File miscPackFile = new File(MinecraftClient.getInstance().runDirectory, getString("resourcePackPath", "resources"));
        ...
        thread = new Thread(new Runnable()
        {
            ...
            private void drawMemoryBar() {
                ...
                glScalef(2, 2, 1);
                glEnable(GL_TEXTURE_2D);
                fontRenderer.draw("Memory Used / Total", 0, 0, 0x000000);
                glDisable(GL_TEXTURE_2D);
                glPopMatrix();
                // border
                glPushMatrix();
...
/**
    ...
    private static class Texture
    {
        ...
        public Texture(Identifier location, Identifier fallback)
        {
            this(location, fallback, true);
        }

        public Texture(Identifier location, Identifier fallback, boolean allowRP)
        {
            InputStream s = null;
            try
            {
    ...
    public static void drawVanillaScreen(TextureManager renderEngine) throws LWJGLException
    {
        if(!enabled)
        {
            MinecraftClient.getInstance().(renderEngine);
        }
    }
    ...
    public static void clearVanillaResources(TextureManager renderEngine, Identifier mojangLogo)
    {
        if(!enabled)
        {
            renderEngine.destroyTexture(mojangLogo);
        }
    }
    ...
    private static InputStream open(Identifier loc, Identifier fallback, boolean allowResourcePack) throws IOException
    {
        if (!allowResourcePack)
            return mcPack.open(ResourceType.ASSETS, loc);

        if(miscPack.contains(ResourceType.ASSETS, loc))
        {
            return miscPack.open(ResourceType.ASSETS,loc);
        }
        else if(fmlPack.contains(ResourceType.ASSETS, loc))
        {
            return fmlPack.open(ResourceType.ASSETS,loc);
        }
        else if(!mcPack.contains(ResourceType.ASSETS, loc) && fallback != null)
        {
