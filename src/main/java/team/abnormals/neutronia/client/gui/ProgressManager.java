...
import team.abnormals.neutronia.utils.ClassNameUtils;
    ...
    public static void pop(ProgressBar bar)
    {
        ...
        if (bar.getSteps() != 0)
        {
            ...
            if (bar.timeEachStep)
            {
                ...
                System.out.println(String.format("Bar Step: %s - %s took %ss", bar.getTitle(), bar.getMessage(), timeString));
            }
            String timeString = String.format("%.3f", ((float) (newTime - bar.startTime) / 1000000 / 1000));
            if (bar.getSteps() == 1)
                System.out.println(String.format("Bar Finished: {} - {} took {}s", bar.getTitle(), bar.getMessage(), timeString));
            else
                System.out.println(String.format("Bar Finished: {} took {}s", bar.getTitle(), timeString));
        }
//        FMLCommonHandler.instance().processWindowMessages();
    }
    ...
    public static class ProgressBar
    {
        ...
        public void step(String message)
        {
            ...
            {
                long newTime = System.nanoTime();
                System.out.println(String.format("Bar Step: %s - %s took %.3fs", getTitle(), getMessage(), ((float)(newTime - lastTime) / 1000000 / 1000)));
                lastTime = newTime;
            }
            step++;
