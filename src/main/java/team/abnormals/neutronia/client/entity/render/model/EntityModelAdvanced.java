...
public class EntityModelAdvanced extends Model {
    ...
    public void updateDefaultPose() {
        this.cuboidList.stream().filter(modelRenderer -> modelRenderer instanceof CuboidAdvanced).forEach(modelRenderer -> {
            CuboidAdvanced advancedModelRenderer = (CuboidAdvanced) modelRenderer;
     ...
     */
    public void faceTarget(float yaw, float pitch, float rotationDivisor, CuboidAdvanced... boxes) {
        ...
        for (CuboidAdvanced box : boxes) {
            box.rotationPointY += yawAmount;
            box.rotationPointX += pitchAmount;
        }
    }
     ...
     */
    public void chainSwing(CuboidAdvanced[] boxes, float speed, float degree, double rootOffset, float swing, float swingAmount) {
        float offset = this.calculateChainOffset(rootOffset, boxes);
        for (int index = 0; index < boxes.length; index++) {
            boxes[index].rotationPointY += this.calculateChainRotation(speed, degree, swing, swingAmount, offset, index);
        }
    }
     ...
     * @param swingAmount the swing amount
     */
    public void chainWave(CuboidAdvanced[] boxes, float speed, float degree, double rootOffset, float swing, float swingAmount) {
        float offset = this.calculateChainOffset(rootOffset, boxes);
        for (int index = 0; index < boxes.length; index++) {
            boxes[index].rotationPointX += this.calculateChainRotation(speed, degree, swing, swingAmount, offset, index);
        }
    }
     ...
     * @param swingAmount the swing amount
     */
    public void chainFlap(CuboidAdvanced[] boxes, float speed, float degree, double rootOffset, float swing, float swingAmount) {
        float offset = this.calculateChainOffset(rootOffset, boxes);
        for (int index = 0; index < boxes.length; index++) {
            boxes[index].rotationPointZ += this.calculateChainRotation(speed, degree, swing, swingAmount, offset, index);
        }
    }
    ...
    private float calculateChainOffset(double rootOffset, CuboidAdvanced... boxes) {
     ...
     */
    public void walk(CuboidAdvanced box, float speed, float degree, boolean invert, float offset, float weight, float walk, float walkAmount) {
     ...
     */
    public void flap(CuboidAdvanced box, float speed, float degree, boolean invert, float offset, float weight, float flap, float flapAmount) {
     ...
     */
    public void swing(CuboidAdvanced box, float speed, float degree, boolean invert, float offset, float weight, float swing, float swingAmount) {
     ...
     */
    public void bob(CuboidAdvanced box, float speed, float degree, boolean bounce, float f, float f1) {
