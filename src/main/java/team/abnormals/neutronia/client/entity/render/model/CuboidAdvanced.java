...
public class CuboidAdvanced extends Cuboid {
    ...
    private EntityModelAdvanced model;
    private CuboidAdvanced parent;
    private int displayList;
    private boolean compiled;

    public CuboidAdvanced(EntityModelAdvanced model, String name) {
        super(model, name);
        this.model = model;
    }

    public CuboidAdvanced(EntityModelAdvanced model) {
        this(model, null);
    }

    public CuboidAdvanced(EntityModelAdvanced model, int textureOffsetX, int textureOffsetY) {
        this(model);
        this.setTextureOffset(textureOffsetX, textureOffsetY);
    }

    @Override
    public Cuboid addBox(String partName, float offX, float offY, float offZ, int width, int height, int depth) {
        partName = this.name + "." + partName;
        TextureOffset textureoffset = this.model.tex(partName);
    ...
    @Override
    public Cuboid addBox(float offX, float offY, float offZ, int width, int height, int depth) {
        this.boxes.add(new Box(this, this.textureOffsetX, this.textureOffsetY, offX, offY, offZ, width, height, depth, 0.0F));
        return this;
    }

    @Override
    public Cuboid addBox(float offX, float offY, float offZ, int width, int height, int depth, boolean mirrored) {
        this.boxes.add(new Box(this, this.textureOffsetX, this.textureOffsetY, offX, offY, offZ, width, height, depth, 0.0F, mirrored));
        return this;
    }
    ...
    public void updateDefaultPose() {
        this.defaultRotationX = this.rotationPointX;
        this.defaultRotationY = this.rotationPointY;
        this.defaultRotationZ = this.rotationPointZ;

        this.defaultOffsetX = this.textureOffsetX;
        this.defaultOffsetY = this.textureOffsetY;
        this.defaultOffsetZ = this.textureOffsetY;
    ...
    public void addChild(ModelRenderer child) {
        ...
        if (child instanceof CuboidAdvanced) {
            CuboidAdvanced advancedChild = (CuboidAdvanced) child;
     ...
     */
    public CuboidAdvanced getParent() {
        return this.parent;
    }
     ...
     */
    public void setParent(CuboidAdvanced parent) {
        this.parent = parent;
    }
    ...
    public EntityModelAdvanced getModel() {
        return this.model;
    }
    ...
    @Override
    public CuboidAdvanced setTextureOffset(int textureOffsetX, int textureOffsetY) {
        this.textureOffsetX = textureOffsetX;
        this.textureOffsetY = textureOffsetY;
        return this;
    }

    public void transitionTo(CuboidAdvanced to, float timer, float maxTime) {
     ...
     */
    public Vec3d getModelPos(CuboidAdvanced modelRenderer, Vec3d recurseValue) {
        double x = recurseValue.x;
        double y = recurseValue.y;
        double z = recurseValue.z;
        Point3d rendererPos = new Point3d(x, y, z);

        CuboidAdvanced parent = modelRenderer.getParent();
        if (parent != null) {
