...
public class SmallRedwoodFeature extends MegaTreeFeature<DefaultFeatureConfig> {
    ...
    public SmallRedwoodFeature(Function<Dynamic<?>, ? extends DefaultFeatureConfig> function_1) {
        super(function_1, false, 13, 15, TreesSubModule.redwood.log.getDefaultState(), TreesSubModule.redwood.leaves.getDefaultState());
    }
        ...
        if (!this.checkTreeFitsAndReplaceGround(modifiableTestableWorld_1, blockPos_1, int_1)) {
            ...
            for (int int_2 = 0; int_2 < int_1; ++int_2) {
                if (isAirOrLeaves(modifiableTestableWorld_1, blockPos_1.up(int_2))) {
                    this.setBlockState(set_1, modifiableTestableWorld_1, blockPos_1.up(int_2), TreesSubModule.redwood.log.getDefaultState(), mutableIntBoundingBox);
                }

                if (int_2 < int_1 - 1) {
                    if (isAirOrLeaves(modifiableTestableWorld_1, blockPos_1.add(1, int_2, 0))) {
                        this.setBlockState(set_1, modifiableTestableWorld_1, blockPos_1.add(1, int_2, 0), TreesSubModule.redwood.log.getDefaultState(), mutableIntBoundingBox);
                    }

                    if (isAirOrLeaves(modifiableTestableWorld_1, blockPos_1.add(1, int_2, 1))) {
                        this.setBlockState(set_1, modifiableTestableWorld_1, blockPos_1.add(1, int_2, 1), TreesSubModule.redwood.log.getDefaultState(), mutableIntBoundingBox);
                    }

                    if (isAirOrLeaves(modifiableTestableWorld_1, blockPos_1.add(0, int_2, 1))) {
                        this.setBlockState(set_1, modifiableTestableWorld_1, blockPos_1.add(0, int_2, 1), TreesSubModule.redwood.log.getDefaultState(), mutableIntBoundingBox);
                    }
                }
            }
