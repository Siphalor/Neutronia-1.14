    ...
    public boolean generate(Set<BlockPos> set_1, ModifiableTestableWorld world, Random random_1, BlockPos blockPos_1, MutableIntBoundingBox mutableIntBoundingBox) {
        ...
        if (blockPos_1.getY() >= 1 && blockPos_1.getY() + height + 1 <= 256 && isNaturalDirtOrGrass(world, blockPos_1.down())) {
                ...
                for (int x = -k; x < k + 1; ++x)
                    for (int z = -k; z < k + 1; ++z) {
                        if (Math.sqrt(Math.pow(x, 2) + Math.pow(z, 2)) < k)
                            setBlockState(set_1, world, blockPos_1.add(x, i, z), TreesSubModule.redwood.leaves.getDefaultState(), mutableIntBoundingBox);
                    }
            }
            ...
            for (int i = -2; i < height - 3; ++i) {
                for (int x = -1; x < 2; ++x)
                    for (int z = -1; z < 2; ++z)
                        setBlockState(set_1, world, blockPos_1.add(x, i, z), TreesSubModule.redwood.log.getDefaultState(), mutableIntBoundingBox);
            }
