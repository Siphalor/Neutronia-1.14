...
import team.hollow.neutronia.blocks.idk.CornerBlock;
import team.hollow.neutronia.blocks.idk.PostBlock;
import team.hollow.neutronia.blocks.idk.SidingBlock;
...
public class PiecesSet {
	...
	public PiecesSet generate() {
		...
		if(shouldHavePiece(BlockPiece.STAIRS)) {
			pieces.put(BlockPiece.STAIRS, new team.hollow.neutronia.blocks.idk.StairsBlock(base.getDefaultState(),Block.Settings.copy(getBase())));
		}
	...
	public PiecesSet register() {
		...
		for(BlockPiece b : pieces.keySet()) {
			Registry.register(Registry.BLOCK, new Identifier(Neutronia.MOD_ID, b.getName(getName())), pieces.get(b));
			BlockItem item = new BlockItem(pieces.get(b), (new Item.Settings()).itemGroup(Neutronia.groups.get(b)));
