...
public class ChestBaseBlockEntityRenderer extends BlockEntityRenderer<ChestBaseBlockEntity> {
    ...
    @Override
    public void render(ChestBaseBlockEntity var1, double var2, double var4, double var6, float var8, int var9) {
    ...
    private ChestEntityModel method_3562(ChestBaseBlockEntity var1, int var2, boolean var3) {
        Identifier var4;
        if (var2 >= 0) {
            var4 = DESTROY_STAGE_TEXTURES[var2];
        } else {
            var4 = var3 ? var1.baseBlock.getDoubleModelTexture() : var1.baseBlock.getModelTexture();
        }
        this.bindTexture(var4);
        return var3 ? this.modelDoubleChest : this.modelSingleChest;
    }

    private void method_3561(ChestBaseBlockEntity var1, float var2, ChestEntityModel var3) {
