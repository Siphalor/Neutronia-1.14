package team.hollow.neutronia.client.book.text;
...
public class BookTextParser {
	...
	static {
		...
		register((parameter, state) -> {
			...
			if (result == null) {
				state.tooltip = I18n.translate("neutronia.gui.lexicon.keybind_missing", parameter);
				return "N/A";
			}

			state.tooltip = I18n.translate("neutronia.gui.lexicon.keybind", I18n.translate(result.getCategory()));
		...
		register((parameter, state) -> {
			...
			if (isExternal) {
				...
				state.tooltip = I18n.translate("neutronia.gui.lexicon.external_link");
	...
	private static KeyBinding getKeybindKey(SpanState state, String keybind) {
		...
		KeyBinding[] keys = state.gui.mc.options.keyBindings;
		for(KeyBinding k : keys) {
			String name = k.getLocalizedName();
