...
import team.hollow.neutronia.client.book.BookEntry;
...
public abstract class GuiBook extends Screen {
	...
	final void drawTooltip(int mouseX, int mouseY) {
		if(tooltipStack != null) {
			...
			if(provider != null && (!(this instanceof GuiBookEntry) || ((GuiBookEntry) this).entry != provider.getLeft())) {
				tooltip.add(TextFormat.GOLD + "(" + I18n.format("patchouli.gui.lexicon.shift_for_recipe") + ')');
				targetPage = provider;
			}
		...
		if(isMouseInRelativeRange(mouseX, mouseY, barLeft, barTop, barWidth, barHeight)) {
			...
			if(unlockedSecretEntries > 0) {
				if(unlockedSecretEntries == 1)
					tooltip.add(TextFormat.GRAY + I18n.format("patchouli.gui.lexicon.progress_tooltip.secret1"));
				else tooltip.add(TextFormat.GRAY + I18n.format("patchouli.gui.lexicon.progress_tooltip.secret", unlockedSecretEntries)); 
			}
			
			if(unlockedEntries != totalEntries)
				tooltip.add(TextFormat.GRAY + I18n.format("patchouli.gui.lexicon.progress_tooltip.info"));
	...
	public void drawCenteredStringNoShadow(String s, int x, int y, int color) {
		fontRenderer.draw(s, x - fontRenderer.getStringWidth(s) / 2, y, color);
	}
