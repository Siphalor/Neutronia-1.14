...
public class ForgeConfigSpec extends UnmodifiableConfigWrapper<Config> {
    ...
    public void setConfig(CommentedConfig config) {
        ...
        if (!isCorrect(config)) {
            ...
            LogManager.getLogger().warn(MarkerManager.getMarker("CORE"), "Configuration file {} is not correct. Correcting", configName);
            correct(config, (action, path, incorrectValue, correctedValue) ->
                    LogManager.getLogger().warn(MarkerManager.getMarker("CORE"), "Incorrect key {} was corrected from {} to {}", DOT_JOINER.join(path), incorrectValue, correctedValue));
