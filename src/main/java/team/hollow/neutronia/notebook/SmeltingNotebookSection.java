...
public class SmeltingNotebookSection implements INotebookSection {
    ...
    public List<INotebookElement> getElements(ClientPlayerEntity player, int page) {
        ...
        if (page == 0) {
            elements.add(new NotebookElement.SmallHeading("notebook.neutronia.smelting.title").withPadding(3));
        } else {
            elements.add(new NotebookElement.Padding(3));
        }

        int firstText = NotebookElement.textPages("notebook.neutronia.smelting.0", 2);
        elements.addAll(NotebookElement.wrapText("notebook.neutronia.smelting.0", 2, 0, page));

        if (page == firstText + 1) {
            elements.add(new NotebookElement.Padding(4));
            elements.add(new NotebookElement.Paragraph(true, 1, "block.neutronia.smelter").withPadding(10));
        ...
        if (page >= firstText + 2) {
            elements.addAll(NotebookElement.wrapText("notebook.neutronia.smelting.1", 0, firstText + 2, page));
        }
        return elements;
    }
    ...
    public int getPageCount(ClientPlayerEntity player) {
        return NotebookElement.textPages("notebook.neutronia.smelting.0", 2) + NotebookElement.textPages("notebook.neutronia.smelting.1", 0) + 2;
    }
}