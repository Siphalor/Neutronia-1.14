...
public class TransfigurationNotebookSection implements INotebookSection {
    ...
    public List<INotebookElement> getElements(ClientPlayerEntity player, int page) {
        ...
        if (page == 0) {
            elements.add(new NotebookElement.SmallHeading("notebook.neutronia.transfiguration.title").withPadding(3));
        } else {
            elements.add(new NotebookElement.Padding(3));
        }

        int firstText = NotebookElement.textPages("notebook.neutronia.transfiguration.0", 2);
        ...
        if (page == firstText + 1) {
            ...
            elements.add(new NotebookElement.Paragraph(true, 1, "block.neutronia.transfiguration_table").withPadding(10));
        ...
        if (page >= firstText + 2) {
            int secondText = NotebookElement.textPages("notebook.neutronia.transfiguration.1", 0) + firstText + 2;
            elements.addAll(NotebookElement.wrapText("notebook.neutronia.transfiguration.1", 0, firstText + 2, page));

            if (page == secondText + 1) {
                elements.add(new NotebookElement.Padding(4));
                elements.add(new NotebookElement.Paragraph(true, 1, "item.neutronia.gold_crystal").withPadding(8));
    ...
    public int getPageCount(ClientPlayerEntity player) {
        return NotebookElement.textPages("notebook.neutronia.transfiguration.0", 2) + NotebookElement.textPages("notebook.neutronia.transfiguration.1", 0) + 3;
    }
}